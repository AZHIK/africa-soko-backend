...
from app.schemas.product_shema import ImageCreate, ImageRead
...
async def add_product_image(
    ...
    if product.store_id not in vendor_stores:
        raise HTTPException(
            status_code=403, detail="Not authorized to add images to this product."
        )

    db_image = ProductImage.model_validate(
        image_data, update={"product_id": product_id}
    )
